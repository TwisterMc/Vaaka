...
final class SiteTab: NSObject {
    ...
    class SiteDownloadHandler: NSObject, WKDownloadDelegate, Cancellable {
        ...
        func download(_ download: WKDownload, decideDestinationUsing response: URLResponse, suggestedFilename: String) async -> URL? {
            Logger.shared.debug("[DEBUG] decideDestinationUsing called for: \(suggestedFilename), response URL: \(response.url?.absoluteString ?? "nil")")
            
            // Auto-download to Downloads folder without showing a save panel
            // This avoids async continuation issues and matches typical browser behavior
            ...
            // Generate a local id for this download
            let id = UUID().uuidString
            self.downloadId = id

            // Register immediately with the destination
            Logger.shared.debug("[DEBUG] Auto-downloading to: \(proposed?.path ?? "nil")")
        ...
        func downloadDidFinish(_ download: WKDownload) {
             DispatchQueue.main.async {
                ...
                // Clear expected interruption marker for this site (download finished successfully)
                if let sid = self.siteTab?.site.id {
                    SiteTabManager.shared.clearExpectedInterruption(siteId: sid)
                }
         ...
         func download(_ download: WKDownload, didFailWithError error: Error, resumeData: Data?) {
             DispatchQueue.main.async {
                ...
                // Clear expected interruption marker for this site (download failed)
                if let sid = self.siteTab?.site.id {
                    SiteTabManager.shared.clearExpectedInterruption(siteId: sid)
                }
                if let id = self.downloadId {
